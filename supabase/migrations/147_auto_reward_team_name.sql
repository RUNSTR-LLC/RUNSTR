-- Migration 147: Enrich auto-reward with team name for push notifications
--
-- Problem: The "Reward Earned!" push notification says "You earned 50 sats for
-- today's workout" but doesn't mention which team/charity the user supports.
-- Including the team name makes the notification more personal and reinforces
-- the charity connection.
--
-- Fix: In the tag extraction loop that already finds the `lightning` tag, also
-- extract the `team` tag. Look up the team name from the user_teams table.
-- Include `team_name` in the v_reward_body JSON so claim-reward can use it
