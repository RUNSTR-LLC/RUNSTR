...
export class WorkoutCacheService {
  ...
  // Cache TTL: 3 minutes (workouts change more frequently than teams)
  ...
  /**
   * Get merged workouts with cache-first strategy
  ...
  async getMergedWorkouts(limit = 100): Promise<WorkoutMergeResult> {
    ...
    if (cachedResult && cachedResult.allWorkouts.length > 0) {
      console.log(
        `✅ WorkoutCacheService: Returning ${cachedResult.allWorkouts.length} cached workouts`
      );

      // Add cache metadata
      cachedResult.fromCache = true;
      cachedResult.cacheAge = cacheTime ? Date.now() - cacheTime : undefined;
  ...
  /**
   * Fetch workouts from services and update cache
   */
  private async fetchAndCacheWorkouts(limit: number): Promise<WorkoutMergeResult> {
    ...
    try {
      ...
      if (result && result.allWorkouts.length > 0) {
        ...
        console.log(
          `✅ WorkoutCacheService: Cached ${result.allWorkouts.length} workouts`
        );
      }
