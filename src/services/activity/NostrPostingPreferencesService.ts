 ...
 *    workout data, rendered natively by Amethyst). Default 'kind1'.
 *
...
export class NostrPostingPreferencesService {
  ...
  /** Which event the Post action produces. Default 'kind1'. */
  static async getPostFormat(): Promise<PostFormat> {
    ...
    try {
      ...
      this.cachedFormat = stored === 'kind1301' ? 'kind1301' : 'kind1';
