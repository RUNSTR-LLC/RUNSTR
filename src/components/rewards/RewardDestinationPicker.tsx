...
interface RewardDestinationPickerProps {
  /** When true, render the option list inline (no Modal/header) so it can live
   *  directly inside a screen. Defaults to modal presentation. */
  inline?: boolean;
  visible?: boolean;
  onClose?: () => void;
...
export const RewardDestinationPicker: React.FC<RewardDestinationPickerProps> = ({
  ...
  // Load user data on mount / when the logged-in user resolves
  useEffect(() => {
    if (visible) {
      loadUserData();
    }
  ...
  const loadUserData = async () => {
    try {
      ...
      // Resolve the logged-in user's real profile name (cache-first) so the
      // YOU row shows them, not the generic "Anonymous Athlete" default.
      if (currentUser?.npub) {
        const profile = await nostrProfileService.getProfile(currentUser.npub);
        const name = profile?.display_name || profile?.name;
        if (name && name !== 'Anonymous Athlete') {
  ...
  // Best available label for the logged-in user — real name if resolved,
  // otherwise a clean "You" rather than the "Anonymous Athlete" placeholder.
  const rawSelfName = currentUser?.displayName || currentUser?.name || '';
  const selfName =
    resolvedName ||
    (rawSelfName && rawSelfName !== 'Anonymous Athlete' ? rawSelfName : 'You');
  ...
  // Inline mode: render the list directly inside the host screen (no Modal,
  // no own ScrollView — flows within the screen's existing scroll).
  if (inline) {
    return (
      <View style={styles.inlineContainer}>
        {headerBlock}
        <View style={styles.inlineBody}>{sections}</View>
        {childModals}
      </View>
    );
  }
...
const styles = StyleSheet.create({
  ...
  // Inline mode (embedded in a host screen, no Modal wrapper)
  inlineContainer: {
    width: '100%',
  },
  inlineBody: {
